    ...
    for asset in _targets(context):
        for param in _params(asset.url):
            ...
            observations.append(SecurityObservation(
                ...
                source="web.xss",
                description=f"Reflected XSS canary checked for parameter {param} on {_safe_url(asset.url)}",
                asset_ids=(asset.id,),
            ...
            if reflected and not encoded:
                findings.append(Finding(
                    ...
                    confidence=0.96,
                    evidence=(oid,),
                    metadata={"severity": "high", "parameter": param, "url": _safe_url(asset.url),
    ...
    for asset in _targets(context):
        for param in _params(asset.url):
            ...
            observations.append(SecurityObservation(
                ...
                source="web.sqli",
                description=f"Non-destructive SQL error differential checked for parameter {param} on {_safe_url(asset.url)}",
                asset_ids=(asset.id,),
            ...
            if errors:
                findings.append(Finding(
                    ...
                    metadata={"severity": "high", "parameter": param,
                              "error_signatures": errors, "url": _safe_url(asset.url)},
                ))
                ...
                findings.append(Finding(
                    ...
                    metadata={"severity": "medium", "parameter": param,
                              "body_length_delta": body_delta, "url": _safe_url(asset.url)},
                ))
    return ModuleResult(observations=tuple(observations), findings=tuple(findings))
        ...
        for pattern, label in _SECRET_PATTERNS:
            ...
            observations.append(SecurityObservation(
                ...
                data={
                    ...
                    "match_digest": sha256(match.group(0).encode("utf-8", errors="replace")).hexdigest()[:16],
                },
                confidence=0.88,
            ))
            findings.append(Finding(
                ...
                confidence=0.78,
                evidence=(oid,),
                metadata={"severity": "high", "pattern": label, "url": _safe_url(asset.url)},
            ))
    return ModuleResult(observations=tuple(observations), findings=tuple(findings))
...
def run_web_api(context):
    ...
    for asset in _targets(context):
        ...
        observations.append(SecurityObservation(
            ...
            source="web.api",
            description=f"API baseline observed for {_safe_url(asset.url)}",
            asset_ids=(asset.id,),
        ...
        if is_json and response.status == 200 and not response.headers.get("cache-control"):
            findings.append(Finding(
                ...
                confidence=0.63,
                evidence=(oid,),
                metadata={"severity": "low", "url": _safe_url(asset.url)},
            ))
    return ModuleResult(observations=tuple(observations), findings=tuple(findings))
