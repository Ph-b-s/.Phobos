...
Adapters translate the shared :class:`ModuleContext` into bounded, structured
module results. Vulnerability logic stays in the underlying runners/analyzers.
"""
...
from assessment_engine import default_indirect_prompt_injection_engine, indirect_prompt_injection_procedure
...
def run_nmap_module(context: ModuleContext) -> ModuleResult:
    ...
    observations = tuple(
        SecurityObservation(
            ...
            source="web.nmap",
            description=(
                f"Nmap observed open {port.protocol.upper()} port {port.port}"
...
class IndirectPromptInjectionAdapter:
    ...
    The target-specific interaction contract is supplied by the planner in
    ``context.metadata['indirect_prompt_injection']``. This keeps the generic
    procedure reusable without inventing application endpoints or selectors.
    """

    REQUIRED = (
        "source_url",
        "source_selector",
        "source_submit_selector",
        "chat_url",
        "chat_input_selector",
    ...
    def handlers(self) -> dict[str, Any]:
        return {
            "discover_chat": self.discover_chat,
            "map_ai_api": self.map_ai_api,
            "map_tool_arguments": self.map_tool_arguments,
            "establish_auth_boundary": self.establish_auth_boundary,
            "discover_indirect_input": self.discover_indirect_input,
            "seed_canary": self.seed_canary,
            "prove_influence": self.prove_influence,
            "validate_impact": self.validate_impact,
        }
    ...
    def discover_chat(self):
        ...
        return (
            Observation(
                "chat_surface",
                f"LLM interaction surface discovered at {snapshot.url}",
                "browser_adapter",
                metadata={"url": snapshot.url, "title": snapshot.title},
            ),
        )

    def map_ai_api(self):
        records = self.browser.network_observations()
        return (
            Observation(
                "tool_inventory",
                "Browser network activity captured while opening the configured LLM surface.",
                "browser_adapter",
                metadata={"network_observations": len(records)},
            ),
        )
    ...
    def map_tool_arguments(self):
        return (
            Observation(
                "tool_arguments",
                "Configured AI interaction selectors provide the bounded tool/input contract.",
    ...
    def establish_auth_boundary(self):
        if not bool(self.config.get("allow_state_change", self.context.metadata.get("allow_state_change", False))):
            return (
                Observation(
                    "authenticated_tool_execution",
                    "Authenticated execution boundary was not exercised because state-changing execution was not explicitly authorized.",
                    "indirect_prompt_injection_adapter",
        ...
        return (
            Observation(
                ...
                "request_manager",
                metadata={"status": response.status, "authorized": True},
            ),
        )
    ...
    def discover_indirect_input(self):
        ...
        return (
            Observation(
                ...
                "request_manager",
                metadata={"url": str(self.config["source_url"]), "status": response.status},
            ),
        )
    ...
    def seed_canary(self):
        ...
        return (
            Observation(
                "canary_seeded",
                "A unique Phobos canary was placed in the configured indirect input source.",
                "browser_adapter",
                evidence=(marker,),
    ...
    def prove_influence(self):
        ...
        self._baseline_text = self._submit_chat(queries[0])
        induced = self._submit_chat(queries[1])
        found = self.canary in induced
        return (
            Observation(
                "baseline_compared",
                "Clean and induced LLM interactions were executed for comparison.",
                "browser_adapter",
                metadata={"baseline_length": len(self._baseline_text), "induced_length": len(induced)},
            ),
            *(
                (
                    Observation(
                        "canary_observed",
                        "The exact seeded canary reappeared in the LLM response.",
                        "browser_adapter",
    ...
    def validate_impact(self):
        if not bool(self.config.get("allow_state_change", self.context.metadata.get("allow_state_change", False))):
            raise ModuleAdapterError("impact validation requires explicit allow_state_change=True")
        if not self.config.get("impact_validation_selector"):
            raise ModuleAdapterError("impact validation requires impact_validation_selector")
        return (
            Observation(
                "state_change_validated",
                "The configured impact-validation control was authorized and identified, but no automatic destructive action is performed by this adapter.",
                "indirect_prompt_injection_adapter",
                metadata={"canary": self.canary, "validated": False},
            ),
        )
...
def run_indirect_prompt_injection_module(context: ModuleContext) -> ModuleResult:
    ...
    run = default_indirect_prompt_injection_engine().run(
    ...
    observations = tuple(
        SecurityObservation(
            ...
            kind=f"ai.indirect_prompt_injection.{item.kind}",
            source="ai.indirect_prompt_injection",
            description=item.description,
            data={"evidence": list(item.evidence), **item.metadata},
    ...
    if run.result.finding_type is not None:
        findings = (
            Finding(
                id="ai.indirect_prompt_injection",
                type=run.result.finding_type,
    ...
    if run.errors:
        raise ModuleAdapterError("; ".join(run.errors))
    return ModuleResult(observations=observations, findings=findings)
