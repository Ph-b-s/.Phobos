...
def default_module_selection(*, include_nmap: bool = False, include_indirect_ai: bool = False) -> ScanPlan:
    selected = [
        ...
        ModuleSelection("web.graphql", "assess discovered GraphQL endpoints with bounded read-only introspection"), ModuleSelection("web.graphql_auth_surface", "identify authorization-sensitive GraphQL surfaces"),
        ModuleSelection("web.ssti", "probe arithmetic-only template expressions for evaluation signals"), ModuleSelection("web.path_traversal", "probe GET parameters with a nonexistent traversal marker"),
        ModuleSelection("web.file_upload", "passively assess discovered upload validation signals"), ModuleSelection("web.websocket", "discover WebSocket endpoints without connecting"),
        ModuleSelection("web.websocket_auth_surface", "inventory WebSocket authentication signals"), ModuleSelection("web.ssrf", "identify likely server-side request sinks"),
        ...
        ModuleSelection("web.host_header", "inspect Host-header trust behavior without changing state"), ModuleSelection("web.object_authorization", "compare explicitly configured owned and peer objects"),
        ...
        ModuleSelection("ai.tool_rag_bridge", "connect AI tool and retrieval surfaces"), ModuleSelection("cross_layer.web_to_ai", "correlate Web inputs with AI surfaces"),
        ModuleSelection("cross_layer.ai_to_web", "correlate AI surfaces with downstream Web capabilities"), ModuleSelection("cross_layer.auth_boundary", "review Web/AI authorization boundaries"),
        ModuleSelection("cross_layer.data_flow", "trace cross-layer data movement"), ModuleSelection("cross_layer.control_flow", "trace AI-mediated control flow"),
        ModuleSelection("cross_layer.capability_escalation", "review capability expansion"), ModuleSelection("cross_layer.attack_path", "rank cross-layer attack paths"),
    ]
    if include_indirect_ai: selected.append(ModuleSelection("ai.indirect_prompt_injection", "run configured indirect-injection assessment"))
    if include_nmap: selected.append(ModuleSelection("web.nmap", "supplemental web-facing service vulnerability check"))
...
def merge_module_selections(base: ScanPlan, additions: Iterable[ModuleSelection], *, source: str | None = None) -> ScanPlan:
    catalog = module_index(); combined = list(base.selections); seen = {item.module_id for item in combined}
    for selection in additions:
        if selection.module_id not in catalog: raise ValueError(f"unknown security module: {selection.module_id}")
        if selection.module_id not in seen: combined.append(selection); seen.add(selection.module_id)
...
def validate_plan(plan: ScanPlan) -> ScanPlan:
    catalog = module_index(); seen: set[str] = set(); validated: list[ModuleSelection] = []; seen_supplemental = False
    for selection in plan.selections:
        spec = catalog.get(selection.module_id)
        if spec is None: raise ValueError(f"unknown security module: {selection.module_id}")
        if selection.module_id in seen: continue
        if spec.stage is ModuleStage.SUPPLEMENTAL: seen_supplemental = True
        elif seen_supplemental: raise ValueError(f"security module {selection.module_id} cannot run after a supplemental module")
        seen.add(selection.module_id); validated.append(selection)
    return ScanPlan(tuple(validated), source=plan.source)


def module_domains(plan: ScanPlan) -> tuple[str, ...]:
    catalog = module_index(); domains: list[str] = []
    for selection in plan.selections:
...
def execute_plan(target: str, assets: tuple[Asset, ...], plan: ScanPlan, *, runners: dict[str, Any] | ModuleRegistry | None = None,
    ...
    if isinstance(runners, ModuleRegistry): registry = runners
    elif runners is None: registry = default_module_registry()
    else:
        registry = default_module_registry()
        for module_id, handler in runners.items(): registry.replace(module_id, handler)
    module_run = ModuleRunner(registry).run(target, (selection.module_id for selection in plan.selections), knowledge=knowledge, graph=graph,
    ...
    return ScanResult(target=target, assets=module_run.knowledge.assets, findings=module_run.findings,
        modules_run=tuple(item.module_id for item in module_run.executions if item.status == "completed"), errors=module_run.errors,
        knowledge=module_run.knowledge, module_run=module_run)


def build_planner_context(target: str, knowledge: KnowledgeStore, *, completed_modules: Iterable[str] = (), iteration: int = 1,
                          eligible_modules: Iterable[str] | None = None) -> str:
    completed = set(completed_modules); allowed = set(eligible_modules or (item.id for item in module_index().values()))
    executable = sorted(item.id for item in module_index().values() if item.active and item.implemented and item.id not in completed and item.id in allowed)
    return json.dumps({"target": target, "iteration": iteration, "completed_modules": sorted(completed), "available_executable_modules": executable,
                       "knowledge": knowledge.context(max_items=400)}, ensure_ascii=False)
