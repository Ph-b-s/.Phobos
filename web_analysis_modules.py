...
def run_web_client_javascript(context):
    ...
    for asset in javascript:
        ...
        observations.append(SecurityObservation(
            id=oid,
            kind="web.client_javascript.analysis",
            source="web.client_javascript",
            ...
            asset_ids=(asset.id,),
            data={"source_signals": sources, "sink_signals": sinks, "source_count": len(sources), "sink_count": len(sinks)},
            confidence=0.86,
        ))
        if sources and sinks:
            findings.append(Finding(
                id=_finding_id("web.client_js.finding", asset.id, "dom-xss"),
                type="potential_dom_xss_source_sink",
                confidence=0.70,
                evidence=(oid,),
                metadata={
                    "severity": "high",
                    "url": _safe_url(asset.url),
                    "source_signals": sources,
                    "sink_signals": sinks,
                    "status": "needs_dataflow_confirmation",
                },
            ))
...
def run_web_jwt(context):
    ...
    for asset in targets:
        ...
        for token in candidates:
            ...
            if alg == "NONE":
                findings.append(Finding(
                    id=_finding_id("web.jwt.finding", asset.id, digest),
                    type="jwt_none_algorithm_signal",
                    confidence=0.98,
                    evidence=(oid,),
                    metadata={"severity": "high", "algorithm": alg, "url": _safe_url(asset.url)},
                ))
    return ModuleResult(observations=tuple(observations), findings=tuple(findings))
...
def run_web_graphql(context):
    ...
    for url in endpoints:
        ...
        observations.append(SecurityObservation(
            id=oid,
            kind="web.graphql.introspection",
            source="web.graphql",
            description=f"GraphQL introspection behavior observed on {_safe_url(url)}",
            data={"status": response.status, "introspection_enabled": introspection_enabled, "content_type": response.headers.get("content-type", "")},
            confidence=0.95,
        ))
        if introspection_enabled:
            findings.append(Finding(
                id=_finding_id("web.graphql.finding", oid),
                type="graphql_introspection_enabled",
                confidence=0.97,
                evidence=(oid,),
                metadata={"severity": "low", "url": _safe_url(url), "validation": "bounded read-only introspection query accepted"},
            ))
    return ModuleResult(observations=tuple(observations), findings=tuple(findings))
