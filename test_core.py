...
def test_scope_rejects_unresolvable_public_host(monkeypatch: pytest.MonkeyPatch):
    ...
    with pytest.raises(ScopeError, match="could not resolve destination safely"):
        s.validate("https://example.com/")


def test_graph_requires_nodes():
    g = Graph()
    g.add_node(id="a", type="input")
    g.add_node(id="b", type="ai_agent")
    g.add_edge(source="a", target="b", relationship="influences")
...
def test_evidence_blocks_traversal(tmp_path):
    ...
    with pytest.raises(ValueError):
        s.write_json("../escape.json", {})


def test_cli_parser():
    a = build_parser().parse_args(
        ["scan", "https://example.com", "--scope", "example.com", "--max-pages", "25"]
    )
